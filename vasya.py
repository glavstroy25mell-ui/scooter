...
PAYME_PASSWORD = "146790"  # Пароль для /payme
MAX_PAYME_ATTEMPTS = 3  # Максимум попыток в день
...
SCOOTER_DATABASE = [
    ...
    # --- Дополнительные модели Kugoo R / Wish / Kirin (ID 101 - 150) ---
...
# Словарь для хранения попыток ввода пароля
payme_attempts = {}
...
# --- РЕГИСТРАЦИЯ МЕНЮ TELEGRAM (БЕЗ /payme) ---
...
async def cb_menu(callback: types.CallbackQuery):
    ...
    # ============ /get - ВЫБИТЬ САМОКАТ (с КД 120 минут) ============
...
# --- СКРЫТАЯ КОМАНДА /payme (НЕ ВИДНА В МЕНЮ) ---
@dp.message(Command("payme"))
async def handle_payme(message: types.Message):
    user_id = message.from_user.id
    ...
    args = message.text.split()
    if len(args) < 2:
        await message.answer("❌ Использование: /payme [сумма_W]")
        return
    ...
    if user_id not in payme_attempts:
        payme_attempts[user_id] = {"attempts": 0, "date": today}
    elif payme_attempts[user_id]["date"] != today:
        payme_attempts[user_id] = {"attempts": 0, "date": today}
    
    if payme_attempts[user_id]["attempts"] >= MAX_PAYME_ATTEMPTS:
        await message.answer(f"❌ Лимит попыток ({MAX_PAYME_ATTEMPTS}) исчерпан на сегодня.")
        return
    
    payme_attempts[user_id]["pending_amount"] = amount
    
    await message.answer(f"🔐 Введите пароль для подтверждения:\n(Осталось попыток: {MAX_PAYME_ATTEMPTS - payme_attempts[user_id]['attempts']})")

@dp.message(F.text)
async def handle_payme_password(message: types.Message):
    user_id = message.from_user.id
    if user_id not in payme_attempts or "pending_amount" not in payme_attempts[user_id]:
        return
    
    today = datetime.now().date()
    if payme_attempts[user_id]["date"] != today:
        payme_attempts[user_id] = {"attempts": 0, "date": today}
    
    if payme_attempts[user_id]["attempts"] >= MAX_PAYME_ATTEMPTS:
        await message.answer(f"❌ Лимит попыток ({MAX_PAYME_ATTEMPTS}) исчерпан на сегодня.")
        del payme_attempts[user_id]["pending_amount"]
        return
    
    payme_attempts[user_id]["attempts"] += 1
    
    entered_password = message.text.strip()
    amount = payme_attempts[user_id]["pending_amount"]
    
    if entered_password != PAYME_PASSWORD:
        remaining = MAX_PAYME_ATTEMPTS - payme_attempts[user_id]["attempts"]
        await message.answer(f"❌ Неверный пароль! Осталось попыток: {remaining}")
        
        if remaining == 0:
            await message.answer("⛔ Попытки исчерпаны на сегодня.")
            del payme_attempts[user_id]["pending_amount"]
        return
    ...
    async with aiosqlite.connect("bot_database.db") as db:
        ...
        await db.execute("UPDATE users SET watts = watts + ? WHERE user_id = ?", (amount, admin_id))
        await db.commit()
    
    del payme_attempts[user_id]["pending_amount"]
    
    admin_display, admin_custom_id = get_user_display_info(admin_id, message.from_user.first_name)
    
    await message.answer(
        f"✅ На баланс зачислено {amount} Ватт.\n"
        f"👤 Получатель: {admin_display} (ID: {admin_custom_id})"
    )
